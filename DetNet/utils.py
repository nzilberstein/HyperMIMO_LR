...
def model_eval(NT, model, snr_min, snr_max, test_batch_size, generator, device, correlated_flag = None, batch_corr = None, rho_low = None, rho_high = None, rho = None, Cu = None, QR=None, test_set_flag = None, test_set = None, iterations=150):
    ...
    #             Q,R,H = createQR(Cu, test_batch_size)
    ...
    #             y, x, j_indices, noise_sigma = generator.give_batch_data(H, NT, snr_db_min=SNR_dBs[i], snr_db_max=SNR_dBs[i], batch_size=test_batch_size)
                    H, y, x, j_indices, noise_sigma = generator.give_batch_data(NT, snr_db_min=SNR_dBs[i], snr_db_max=SNR_dBs[i], batch_size=test_batch_size, QR = QR, Cu = Cu)
            else:

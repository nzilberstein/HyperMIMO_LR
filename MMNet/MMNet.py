...
class MMNet(nn.Module):
    ...
    def __init__(self, num_layers, NT, NR, batch_size, constellation, device='cuda'):
    ...
    def forward(self, H, y, noise_sigma):
        ...
        x_prev = torch.zeros(self.batch_size, x_size, dtype=torch.double).to(device=self.device)
        x_list=[x_prev]

        for index,MMNetbase in enumerate(self.MMNetbase):
            xout = MMNetbase.forward(H, y, x_list[-1].double(), noise_sigma, self.batch_size)
